...
from transformers.models.pix2struct.configuration_pix2struct import (
    Pix2StructConfig,
...
class Pix2StructVisionAttention(nn.Module):
        ...
        if position_bias is None:
            ...
            if attention_mask is None:
                attention_mask = torch.ones(
                    (batch_size, seq_length), device=scores.device, dtype=scores.dtype
                )
    ...
    def _flash_fwd(self, query_states, key_states, value_states, attention_mask):
        # query/key/value states are of shape (batch, num_heads, sequence, head_dim)
        qkv = torch.stack(
            [
                query_states,
                key_states,
        ...
        # qkv is now of shape (batch, seq, 3, num_heads, head_dim)

        key_padding_mask = attention_mask > 0

        attn_output = self.flash(
            qkv,
            causal=False,
    ...
    def forward(
        ...
        if self.flash is None:
            attn_output, attn_weights = self._regular_fwd(
                query_states,
                key_states,
                value_states,
                batch_size,
            ...
            attn_output, attn_weights = self._flash_fwd(
                query_states,
                key_states,
                value_states,
                attention_mask,
...
class Pix2StructVisionModel(Pix2StructPreTrainedModel):
    ...
    def _set_gradient_checkpointing(
        self, module: Pix2StructVisionEncoder, value: bool = False
    ) -> None:
...
class Pix2StructTextModel(Pix2StructPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                Pix2StructTextBlock(config, has_relative_attention_bias=bool(i == 0))
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i, (layer_module, past_key_value) in enumerate(
            zip(self.layer, past_key_values)
        ):
